...
void simpanDataKeFileCSV(struct Pengiriman data[], int jumlah_data) {
    ...
    if (file != NULL) {
        ...
        for (int i = 0; i < jumlah_data; i++) {
            char timestamp_str[20];
            strftime(timestamp_str, sizeof(timestamp_str), "%Y-%m-%d %H:%M:%S", localtime(&data[i].timestamp));
        printf("Writing to file: %s;%s;%.2f;%s;%s;%.2f;%s;%s\n", data[i].nama_pengirim, data[i].tujuan_pengiriman, data[i].berat_barang, data[i].nama_penerima, data[i].kode_resi, data[i].harga, timestamp_str, data[i].status);
        fprintf(file, "%s;%s;%.2f;%s;%s;%.2f;%s;%s\n", data[i].nama_pengirim, data[i].tujuan_pengiriman, data[i].berat_barang, data[i].nama_penerima, data[i].kode_resi, data[i].harga, timestamp_str, data[i].status);
        }
...
void bacaDataDariFile(struct Pengiriman data[], int *jumlah_data) {
    ...
    if (file != NULL) {
       while (fscanf(file, "%49[^;];%49[^;];%f;%49[^;];%9[^;];%f;%ld;%19[^\n]\n", data[*jumlah_data].nama_pengirim,
              data[*jumlah_data].tujuan_pengiriman, &data[*jumlah_data].berat_barang,
              data[*jumlah_data].nama_penerima, data[*jumlah_data].kode_resi, &data[*jumlah_data].harga, &data[*jumlah_data].timestamp, data[*jumlah_data].status) == 8) {
    (*jumlah_data)++;
}
